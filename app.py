...
uploaded_file = st.file_uploader(
    "Upload your merged dataset (e.g., movielens_merged.csv)", type=["csv"]
)


# دالة حساب التوصيات من كود الكولاب
def recommend_movies(user_id, df, k=5, top_n=5):
  # إنشاء مصفوفة التقييمات (User-Item Matrix)
  ...
  # تطبيع التقييمات (Mean-centering)
  ...
  # تحديد الأفلام التي لم يشاهدها المستخدم
  user_ratings = user_item_matrix.loc[user_id]
  unrated_movies = user_ratings[user_ratings.isna()].index

  # جيران المستخدم الأقرب
  ...
  # حساب التقييم المتوقع لكل فيلم غير مشاهد
  ...
  rec_df = (
      pd.DataFrame(list(predicted_ratings.items()), columns=["Title", "Predicted Rating"])
